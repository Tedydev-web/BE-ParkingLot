...
public partial class Program
{
    ...
    private static void ConfigureServices(WebApplicationBuilder builder)
    {
        ...
        builder.Services.AddCors(options =>
        {
            options.AddDefaultPolicy(policy =>
                policy.WithOrigins(builder.Configuration.GetSection("CorsSettings:Origins").Get<string[]>() ?? Array.Empty<string>())
                    .AllowAnyMethod()
                    .AllowAnyHeader()
                    .AllowCredentials());
        });
        ...
        .AddJwtBearer(options =>
        {
            ...
            options.Events = new JwtBearerEvents
            {
                ...
                OnTokenValidated = context =>
                {
                    ...
                    logger.LogInformation($"User roles: {string.Join(", ", user?.Claims.Where(c => c.Type == ClaimTypes.Role).Select(c => c.Value))}");
                    return Task.CompletedTask;
                }
            };
        });
    ...
    private static void ConfigureMiddleware(WebApplication app)
    {
        ...
        app.UseCors(); // Use default policy instead of "AllowAll"
