...
namespace ParkingLot.API.Controllers
{
    ...
    public class ParkingLotController : ControllerBase
    {
        ...
        public async Task<ActionResult<ParkingLotDto>> Create([FromBody] CreateParkingLotDto dto)
        {
            try
            {
                var parkingLot = _mapper.Map<ParkingLot>(dto);
                parkingLot.OwnerId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
                await _parkingLotService.CreateAsync(parkingLot);
                var resultDto = _mapper.Map<ParkingLotDto>(parkingLot);
        ...
        public async Task<ActionResult<ParkingLotDto>> Update(string id, [FromBody] UpdateParkingLotDto dto)
        {
            try
            {
                ...
                await _parkingLotService.UpdateAsync(parkingLot);
                var resultDto = _mapper.Map<ParkingLotDto>(parkingLot);
                return Ok(resultDto);
            }
            catch (Exception ex)
